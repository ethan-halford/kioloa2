...
-   **4. Example - writing basic function**
    -   gl.document - creates roxygen documentation
-   **5. Github specifically for dartR**
    -   fork directory
    -   create local git repo on rstudio
    -   create function - compile local copy - make sure package is installable 
...
```{r,define_function_4,eval=FALSE}
...
```{r,define_function,exercise=T,eval=FALSE}
# SET VERBOSITY
check_one <- gl.basic.stats(possums.gl,verbose = 4)
```
...
```{r badexample_5,exercise=T,eval=FALSE}
...
```{r badexample,exercise=T,eval=FALSE}
# FLAG SCRIPT START
...
```{r badexample_3,exercise=T,eval=FALSE}
# DISPLAY GRAPHICS
 # Boxplot
...
![](images/Repo_fork.png){width=100%}
...
![](images/Fork_yeah.png){width=100%}
...
### 3. Example function - gl.document
